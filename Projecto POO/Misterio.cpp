...
HMisterio::HMisterio(int x,int y,int inst,string t):habitantes(2,2,x,y,t,inst){

}

void HMisterio::Accao(jogo *j,int hab,faccoes &fac,int l,int a,vector <faccoes> &vfac,int nfac,vector <artefactos*> &vart){ 
	...
	{ vfac[nfac].setvechab(j->adicionarhab(vfac[facc].gethabx(i),vfac[facc].gethaby(i),vfac[facc].getHabTipo(i))); 
...
void HMisterio::consumir_comida(faccoes &fac){

	if(fac.getComida() < 0)
		fac.setComida(0);
	else
		fac.setComida( fac.getComida() - 2 );
...
//1- sim, 0- nao
int HMisterio::verificar_cidade(jogo *j,cidade *cdd, faccoes &fac, int nhab){
...
void HMisterio::atribuir_cidade_base(jogo *j, cidade *cdd, faccoes &fac, int nhab){
	for(int i=0; i<j->getTotalCidades(); i++)
		if( cdd[i].getPosX() == fac.gethabx(nhab) )
			if( cdd[i].getPosY() == fac.gethaby(nhab) )
