...
Soldado::Soldado(int x,int y,int inst,string t):habitantes(3,2,x,y,t,inst){

}

void Soldado::Accao(jogo *j,int hab,faccoes &fac,int l,int a,vector <faccoes> &vfac,int nfac,vector <artefactos*> &vart){ 
...
void Soldado::consumir_comida(faccoes &fac){
...
//1- sim, 0- nao
int Soldado::verificar_cidade(jogo *j,cidade *cdd, faccoes &fac, int nhab){
...
void Soldado::atribuir_cidade_base(jogo *j, cidade *cdd, faccoes &fac, int nhab){
	for(int i=0; i<j->getTotalCidades(); i++)
		if( cdd[i].getPosX() == fac.gethabx(nhab) )
			if( cdd[i].getPosY() == fac.gethaby(nhab) )
