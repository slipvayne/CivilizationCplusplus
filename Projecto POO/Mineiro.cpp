...
Mineiro::Mineiro(int x,int y,int inst,string t):habitantes(2,2,x,y,t,inst){

}

void Mineiro::Accao(jogo *j,int hab,faccoes &fac,int l,int a,vector <faccoes> &vfac,int nfac,vector <artefactos*> &vart){ 

	for(int z=0; z <(signed)vart.size(); z++) { //se pisou algum artefacto
...
void Mineiro::consumir_comida(faccoes &fac){
...
//1- sim, 0- nao
int Mineiro::verificar_cidade(jogo *j,cidade *cdd, faccoes &fac, int nhab){
...
void Mineiro::atribuir_cidade_base(jogo *j, cidade *cdd, faccoes &fac, int nhab){
	for(int i=0; i<j->getTotalCidades(); i++)
		if( cdd[i].getPosX() == fac.gethabx(nhab) )
			if( cdd[i].getPosY() == fac.gethaby(nhab) )
